var INFO={id:'streamtape',name:'Streamtape',version:'1.0.0',hosts:['streamtape.com', 'streamtape.to']};
...
async function extract(url, opts) { try { var h=await fetch(url,{headers:{Referer:url}}); var u=pick(h.body,url); if(!u){var m=(h.body||'').match(/robotlink[^>]*>([^<]+)/i); if(m)u='https:'+m[1].replace(/\s+/g,'');} return u?result(u,url):[]; } catch (_) { return []; } }
