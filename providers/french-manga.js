// French-Manga provider for Zangetsu.
...
function request(path, opts) {
  opts = opts || {}; var candidates = BASES.map(function (b) { return String(path).indexOf('http') === 0 ? path.replace(/^https?:\/\/[^/]+/, b) : b + (String(path).charAt(0) === '/' ? path : '/' + path); });
...
function getInfo() { return { name: 'French-Manga', lang: 'fr', baseUrl: SITE, logo: SITE + '/favicon.ico', type: 'anime', version: '1.0.4' }; }
...
function directVideo(u, ref) {
  return fetch(u, { headers: { Referer: ref || SITE, 'User-Agent': UA }, timeoutMs: 20000 }).then(function (r) {
    var rawBody = responseBody(r), decoded = unpackPacked(rawBody), m = decoded.match(/https:[^"' ]+\.(?:m3u8|mp4)(?:\?[^"' ]*)?/i), stream = m && m[0] ? m[0].split(String.fromCharCode(92) + '/').join('/') : '';
    ...
    return [{ url: stream, quality: (stream.match(/(?:2160|1080|720|480|360)p?/i) || ['Unknown'])[0], container: stream.indexOf('.m3u8') >= 0 ? 'hls' : 'mp4', headers: { Referer: u, 'User-Agent': UA }, kind: 'sub' }];
...
function resolveVideo(u, page) { var x = String(u || ''), host = x.indexOf('://') >= 0 ? x.slice(x.indexOf('://') + 3).split('/')[0].toLowerCase().replace(/^www\./, '') : ''; if (host === 'vidzy.org' || host === 'luluvdo.com' || host === 'lulustream.com') return directVideo(u, page); return extractVideo(u, { headers: { Referer: page, 'User-Agent': UA } }).catch(function () { return []; }); }
function getVideoSources(episodeUrl) {
  ...
  return request('/engine/ajax/manga_episodes_api.php?id=' + esc(idOf(page)), {}).then(function (r) {
    ...
    if (item && typeof item === 'object') Object.keys(item).forEach(function (k) { if (typeof item[k] === 'string' && item[k].indexOf('http') === 0) links.push(item[k]); });
